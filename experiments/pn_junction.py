...
class Pn_junction(Experiment):
    ...
    def process(self):
        ...
        self.result = Matrl_const(float(round_dec(model.intercept_, 3)),
                    f'{float(round_dec(-model.coef_[0], 5) * 1000)} × 10ˉ³')

        # 设置曲线图样式
        plt.xticks(np.arange(25, 80, 5))
        plt.yticks(np.arange(float(round_dec(self.data['U/V'].min(), 2)),
    ...
    def draw(self):
        ...
        self.fig.savefig(os.path.join(getPrefix(__file__, -2), "pic", 'pn结温度-电压特性的测定.png'))
