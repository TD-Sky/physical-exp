...
class Solar_battery(Experiment):
    ...
    def process(self):
        ...
        plt.yticks(np.arange(0, float(round_dec(self.data['I/mA'].max(), 1))+0.1, 0.1))
        plt.xlabel('U/V')
        plt.ylabel('I/mA')
        plt.grid()
    ...
    def draw(self):
        ...
        self.fig.savefig(os.path.join(getPrefix(__file__, -2), 'pic', '太阳能电池伏安特性曲线.png'))
    ...
    def output(self) -> str:
        ...
        return R_P + f"最大输出功率: {float(round_dec(Pmax, 3))}\n" + f"相应电阻值: {cores_R}\n" + "手算填充因子，F = Pm/(Isc × Uoc)，请：\n"
